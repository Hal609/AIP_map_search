...
def plot_graph(  # noqa: PLR0913
    ...
    dpi: int = 300,
    mark_nodes: Iterable[int] | None = None,
    ...
    if max_node_size > 0:
        ...
        scatter = ax.scatter(  # type: ignore[union-attr]
            x=gdf_nodes["x"],
            y=gdf_nodes["y"],
            s=node_size,
            c=node_color,
        ...
        if mark_nodes is not None:
            for node in mark_nodes:
                ax.scatter(
                    x=gdf_nodes.loc[node]["x"],
                    y=gdf_nodes.loc[node]["y"],
                    s=node_size*2,
                    c="y",
                    alpha=node_alpha,
                    edgecolor=node_edgecolor,
        ...
        if highlight_node is not None:
            ax.scatter(
                x=gdf_nodes.loc[highlight_node]["x"],
                y=gdf_nodes.loc[highlight_node]["y"],
                s=node_size*4,
                c="r",
                alpha=node_alpha,
                edgecolor=node_edgecolor,
